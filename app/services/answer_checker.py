...
class AnswerChecker:
    ...
    def _has_mode_drift(self, mode: str, text: str) -> bool:
        ...
        if mode == "bridge":
            return not any(k in text for k in ["先", "不急", "慢慢", "顺着", "带你"])

        if mode == "commentary":
            return not any(k in text for k in ["判断", "本质", "问题", "所以", "我先直接"])

        if mode == "persona":
            return not any(k in text for k in ["你", "我", "跟我", "咱", "我们"])
